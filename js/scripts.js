...
function randomNumber(m,n){
    m = parseInt(m);
    n = parseInt(n);
	var out = Math.floor( Math.random() * (n - m + 1) ) + m;
    return out;
};

	window.dig1 = randomNumber(1,20);
	window.dig2 = randomNumber(1,20);

function checkCaptcha(captcha) {
	if (captcha == window.dig1+window.dig2) { 
		return true; 
	}
	else {
		return false;
